...
from app import app, db, User, Layers
...
#  LAYERS TABLE VIEW
#
class Layers_View(ModelView):

    def is_accessible(self):
        #if session.get('login'):
        if session.get('login') and session.get('admin'): # admins can edit, but normal users can access via ../layerc(losed)
          #print("Layers_View is_accessible")
          print("Layers_View accessible login response: {}".format(session.get('login')))
          return True 
        else:
          return False  # no access, protects

    def inaccessible_callback(self, name, **kwargs): # instead of error message, redirect to admin.index in class MyAdminIndexView()
...
class superUser_View(ModelView):
    ...
    column_exclude_list = ['date_joined', 'password_'] # exclude on list view a.k.a initial page
...
# DONT DELETE SELF super account
    def delete_model(self, model):
        ...
        return super(superUser_View, self).delete_model(model)

### end class superUser_view

### start admin view for user table
class adminUser_View(ModelView):

    def is_accessible(self):
        #if session.get('login') and session.get('super'): #1 test - works
        if session.get('login') and session.get('admin') and not session.get('super'):
          print("\nadminUser_View accessible login response: {}".format(session.get('login')))
          return True 
        else:
          return False  # no access, protects
    ...
    def get_query(self):
          return self.session.query(self.model).filter(self.model.is_superuser==False)

    def get_count_query(self):
      return self.session.query(func.count('*')).filter(self.model.is_superuser==False)
    ...
    def delete_model(self, model):
        ...
        return super(adminUser_View, self).delete_model(model)
    ...
    def update_model(self, form, model):
        ...
        return super(adminUser_View, self).update_model(form, model)
...
# this controls indexes
    ...
    def login_view(self):
        ...
        if request.method == 'POST':
            ...
            if user != None and ( user.is_admin or user.is_superuser ):
                ...
                if user.verify_password(user_details['password']):
                    ...
                    flash(
                        "Welcome " +
                        session['firstName'] +
                        "! U bent succesvol ingelogd", "success" + " Your layers activated."
                    )
...
# END of login
    ...
    def logout_view(self):
        ...
        flash(
           "Bye " + name +
           "! U bent succesvol uitgelogd" , "success" 
        )
...
from app import url_root_path

admin = admin.Admin(app, name='Panel', index_view=MyAdminIndexView(), base_template='base_admin.html', template_mode='bootstrap3')

admin.add_view(superUser_View(User, db.session, endpoint='superUser')) # users table
admin.add_view(adminUser_View(User, db.session, endpoint='adminUser')) # users table

admin.add_view(Layers_View(Layers, db.session))  # Layers

admin.add_link(MenuLink(name='Go to Atlas', category='', url=url_root_path, target="_blank"))
