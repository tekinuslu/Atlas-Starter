...
#        self.is_superuser = is_superuser
    ...
    username = db.Column(db.String(64),
                      ...
                      unique=True,
                      nullable=False)
    password_ = db.Column(db.String(128),  # hashed also
                      nullable=False)   # see hybrid_property for functionality change
    ...
    is_active = db.Column(db.Boolean,
                      ...
                      nullable=False,
                      default=False)
    is_admin = db.Column(db.Boolean,       # admin layer credential, controls only layers not users 
                      index=False,
                      unique=False,
                      nullable=False,
                      default=False)
    is_staff = db.Column(db.Boolean,
                      ...
                      nullable=False,
                      default=False)
    is_superuser = db.Column(db.Boolean,      # super user credential, controls both users and layer tables
                      index=False,
                      unique=False,
                      nullable=False,
                      default=False)
...
class Layers(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True) # primary keys are required by SQLAlchemy
    #
    layer_id = db.Column('layer_id',db.String(128),
                      index=False,
                      unique=True,
                      nullable=False,
                      default="") # id_???
    ...
    #
    layer_type = db.Column( db.String(64), nullable=True, default="themelayer:true") # temp solution!

    meta_name = db.Column('meta_name', db.String(128), nullable=True)
    meta_kind = db.Column('meta_kind', db.String(128), nullable=True)
    meta_org = db.Column('meta_org', db.String(256), nullable=True, 
                                                default="mijnDatalab")
    ...
    #
    layer_name = db.Column('layer_name', db.String(128), nullable=True, default='topp:mylayer_???')
    #
    # _popup_attributes --> attribute list in Js :  make list of layer field as addable to this field, 1) ajax call to get fields
    _popup_attributes = db.Column(
         '_popup_attributes', db.String(250), nullable=True)
    # 
    _search_fields = db.Column(
         '_search_fields', db.String(250), nullable=True)
    # 
    ...
    url = db.Column(
         'url',
         db.String(500),
         default='https://mijndatalab.nl/geoserver/topp/wms?', nullable=False)
    # 
    ...
    # 
    server_type = db.Column('server_type', db.String(50), default='geoserver', nullable=False)
    # 
    closed_dataset = db.Column('closed_dataset', db.Boolean, nullable=False, default=True)
    # 
...
# 
    ...
    def params(self):
         if self.closed_dataset == True:
          return "{{ 'layers': '{0}', 'authkey' : '{1}' }}".format(self.layer_name, self.authkey)
         else:
          return "{{'layers': '{0}'}}".format(self.layer_name)
# 
    @property
