...
try: 
  ...
  print("  > Environment APP_SETTINGS is not set or accessible, reading local config file instead")
  app.config.from_object("config_local.DevelopmentConfig")

# init SQLAlchemy so we can use it later in our models
db = SQLAlchemy(app) 

from models import User, Layers #  User@sqlite, Layers@postgres
...
def layersopen():
    datalist = Layers.query.order_by(Layers.ordering).filter_by(closed_dataset=False).all()
...
#    pprint(app.__dict__)
    ...
    return render_template('index.html', hostname=app.config['HOST'], apppath=url_root_path)
        ...
        if user != None:
            ...
            if user.verify_password(user_details['password']):
                ...
                flash(
                    "Welcome " +
                    session['firstName'] +
                    "! U bent succesvol ingelogd," + " uw kaartlagen geactiveerd.", "success"
                )
